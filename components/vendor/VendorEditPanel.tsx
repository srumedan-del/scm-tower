...
import { upsertVendor, deleteVendor } from '@/app/(app)/vendor/actions'
...
const empty: Omit<Vendor, 'id'> = {
  vendor_code: '', vendor_name: '', vendor_type: 'TRUCKING',
  pic_name: '', phone: '', email: '', coverage_area: '',
  default_sla: null, is_active: true,
}
...
export default function VendorEditPanel({
  ...
  const up = (k: keyof typeof form, v: any) =>
    setForm((f) => ({ ...f, [k]: v }))
  ...
  function save() {
    startSaving(async () => {
      ...
      const payload = {
        ...
        default_sla:   form.default_sla == null || Number.isNaN(Number(form.default_sla))
                         ? null : Number(form.default_sla),
      ...
      try {
        await upsertVendor({ id: vendor?.id ?? 0, ...payload })
        onSaved()
        onClose()
      } catch (e: any) {
        setErr(e.message)
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          <Field label="KODE VENDOR *">
            <input
              ...
              onChange={(e) => up('vendor_code', e.target.value)}
              className="inp font-mono"
              placeholder="VOTH001801"
              disabled={!!vendor}
            />
          </Field>
          <Field label="NAMA VENDOR *">
            <input
              ...
              onChange={(e) => up('vendor_name', e.target.value)}
              className="inp"
              placeholder="PT. ..."
            />
          </Field>
          <Field label="TIPE">
            <select
              value={form.vendor_type ?? 'TRUCKING'}
              onChange={(e) => up('vendor_type', e.target.value)}
              className="inp"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="PIC NAME">
              <input value={form.pic_name ?? ''} onChange={(e) => up('pic_name', e.target.value)} className="inp" />
            </Field>
            <Field label="PHONE">
              <input value={form.phone ?? ''} onChange={(e) => up('phone', e.target.value)} className="inp" placeholder="+62 ..." />
            </Field>
          </div>
          <Field label="EMAIL">
            <input value={form.email ?? ''} onChange={(e) => up('email', e.target.value)} className="inp" placeholder="pic@example.com" />
          </Field>
          <Field label="COVERAGE AREA">
            <input value={form.coverage_area ?? ''} onChange={(e) => up('coverage_area', e.target.value)} className="inp" placeholder="MEDAN, BANDA ACEH" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="DEFAULT SLA (HARI)">
              <input
                type="number" min={0}
                value={form.default_sla ?? ''}
                onChange={(e) => up('default_sla', e.target.value === '' ? null : Number(e.target.value))}
                className="inp"
                placeholder="2"
              />
            </Field>
          ...
          {err && <div className="text-red-600 text-sm bg-red-50 rounded p-2">{err}</div>}
        </div>
        ...
        <div className="flex gap-2 justify-between border-t border-border p-4 bg-gray-50 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={save}
              disabled={saving || !form.vendor_code || !form.vendor_name}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50 hover:bg-blue-500"
            >
